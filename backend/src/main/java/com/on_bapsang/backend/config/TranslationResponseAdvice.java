...
public class TranslationResponseAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body,
        ...
        if (lang == null || lang.equalsIgnoreCase("KO")) return body;

        try {
            if (!hasTranslatableFields(body)) return body;
    ...
    private boolean hasTranslatableFields(Object obj, Set<Object> visited) {
        ...
        if (obj instanceof Iterable<?> iterable) {
            for (Object element : iterable) {
                if (hasTranslatableFields(element, visited)) return true;
            }
        } else if (!isSkippable(obj)) {
            for (Field field : obj.getClass().getDeclaredFields()) {
                field.setAccessible(true);
                try {
                    if (field.isAnnotationPresent(Translatable.class)) return true;
                    Object fieldValue = field.get(obj);
                    if (hasTranslatableFields(fieldValue, visited)) return true;
